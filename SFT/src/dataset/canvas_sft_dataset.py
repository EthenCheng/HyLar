...
from .canvas_data_utils import *


# ========== Dataset Class ==========
class SupervisedDatasetCanvas(Dataset):
    def __init__(self, data_root):
        super().__init__()
        self.exclude = ('3D', 'Chemistry', 'Competitive', 'Graph', 'Physics', 'ARC-AGI', 'Ciphers', 'Tetris', 'Connect')
...
class CanvasDataCollator:
    """Collate examples for Stage 1 training with batch-level optimization."""
    
    def __init__(self, processor, canvas_processor, args):
        self.processor = processor
        self.canvas_processor = canvas_processor
        self.args = args
        
        # Precompute token IDs once
        self.canvas_token_idx = processor.tokenizer("<|canvas|>", return_tensors="pt")["input_ids"][0]
        self.canvas_start_idx = processor.tokenizer("<|canvas_start|>", return_tensors="pt")["input_ids"][0]
        self.canvas_end_idx = processor.tokenizer("<|canvas_end|>", return_tensors="pt")["input_ids"][0]
    ...
    def __call__(self, raw_examples):
        ...
        canvas_pixel_values = assistant_batch.get('pixel_values', None)
        canvas_image_grid_thw = assistant_batch.get('image_grid_thw', None)

        new_input_ids, new_attention_mask = process_batch(
            batch["input_ids"], batch["attention_mask"], 
            self.canvas_start_idx, self.canvas_end_idx,
            self.canvas_token_idx, self.args.canvas_token_num, 
        ...
        labels = generate_labels_after_multi_token_start(
            ...
            self.pad_token_idx, self.canvas_token_idx
        )
        batch["labels"] = labels
        
        if canvas_pixel_values is not None:
            canvas_image_mask = mask_image_output_tokens(
                batch["input_ids"], self.canvas_start_idx, self.canvas_token_idx
            )
            batch["canvas_image_mask"] = canvas_image_mask
            ...
            canvas_inputs = self.canvas_processor(images=canvas_images, do_convert_rgb=True, return_tensors="pt")
            batch['canvas_pixel_values'] = canvas_inputs['pixel_values']
            
        return batch


def make_supervised_data_module_canvas(processor, canvas_processor, args):
    """Make dataset and collator for Zebra-CoT training."""
    
    dataset = SupervisedDatasetCanvas(data_root=args.data_path)

    data_collator = CanvasDataCollator(
        processor=processor,
