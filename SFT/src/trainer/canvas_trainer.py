...
def get_canvas_proj_maybe_zero_3(named_params, keys_to_match):
    to_return = {k: t for k, t in named_params if any(key_match in k for key_match in keys_to_match)}
    to_return = {k: maybe_zero_3(v, ignore_status=True, name=k).cpu() for k, v in to_return.items()}
    return to_return

class CanvasSFTTrainer(Trainer):

    def __init__(self, *args, **kwargs):
        super(CanvasSFTTrainer, self).__init__(*args, **kwargs)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.canvas_lr is not None:
                lr_mapper["canvas"] = self.args.canvas_lr
                canvas_parameters = [name for name, _ in opt_model.named_parameters() if "canvas" in name]

            if len(lr_mapper) > 0:
                special_lr_parameters = merger_parameters + visual_parameters + canvas_parameters
                ...
                if canvas_parameters: 
                    optimizer_grouped_parameters.extend(
                        [
                            {
                                "params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and n in canvas_parameters and p.requires_grad)],
                                "weight_decay": self.args.weight_decay,
                                "lr": self.args.canvas_lr,
                            },
                            {
                                "params": [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and n in canvas_parameters and p.requires_grad)],
                                "weight_decay": 0.0,
                                "lr": self.args.canvas_lr,
                            },
                        ]
    ...
    def _save_checkpoint(self, model, trial):
        ...
        if self.args.lora_enable:
            ...
            super(CanvasSFTTrainer, self)._save_checkpoint(model, trial)


    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        Compute training loss:
        1. CE Loss: standard language modeling loss
        2. Canvas Loss: next canvas embedding prediction (MSE or Cosine)
        """

        if 'canvas_pixel_values' in inputs:
            canvas_pixels = inputs.pop('canvas_pixel_values')
            canvas_extractor = getattr(model, "module", model).canvas_extractor
            inputs['canvas_embedding'] = canvas_extractor(canvas_pixels)
        else:
            inputs['canvas_embedding'] = None
        ...
        if inputs["canvas_embedding"] is not None:
            predict_embeddings = outputs.hidden_states

            canvas_image_mask = inputs["canvas_image_mask"]

            shift_image_mask = canvas_image_mask[:, -(predict_embeddings.shape[1] - 1) :].to(predict_embeddings.device)
