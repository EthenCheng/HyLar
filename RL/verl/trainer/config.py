...
class DataConfig:
    ...
    mini_rollout_batch_size: Optional[int] = None  # PAPO-style mini batch size for rollout
...
class AlgorithmConfig:
    ...
    # PAPO-style parameters
    online_filtering: bool = False
    """use online filtering to filter samples based on reward"""
    ...
    """loss average mode: 'token' or 'seq'"""
    # ===== D-1 + D-2: Decoupled Hybrid PPO + closed-form vMF KL =====
    enable_decoupled_hybrid_ppo: bool = False
    """enable D-1 Decoupled Hybrid PPO (separate token/latent surrogate loss + dual clip)"""
    latent_clip_ratio_low: float = 0.2
    """PPO clip ratio lower bound for latent positions"""
    latent_clip_ratio_high: float = 0.28
    """PPO clip ratio upper bound for latent positions"""
    latent_clip_ratio_dual: float = 3.0
    """dual-clip PPO constant for latent positions"""
    latent_loss_alpha: float = 1.0
    """weight coefficient alpha of latent PPO loss relative to token PPO loss"""
    enable_latent_vmf_kl: bool = False
    """enable D-2 closed-form vMF KL constraint (replaces sample-based KL at latent positions)"""
    latent_kl_coef: float = 1e-3
    """coefficient beta_lat for latent closed-form vMF KL"""
    # Answer tag filtering: discard trajectories missing or containing multiple <answer>...</answer> tags
    answer_tag_filtering: bool = False
    """enable <answer> tag filtering: discard entire trajectory if any response in the group is missing or has multiple <answer>...</answer> tags"""
...
class TrainerConfig:
    ...
    """maximum number of tries to make a batch when using online filtering"""
    
    # Sample saving config
    save_samples: bool = True
    """whether to save rollout samples to JSON files"""
