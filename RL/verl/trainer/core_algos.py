...
def compute_grpo_step_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6,
    step_end_poss = None, delim_poss=None, normalize: bool = True
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO, operating on step reward
    (with only one scalar reward for each response).
    ...
    Args:
        ...
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        step_end_poss: `List[List]` 
            the end position of each step in the response (the positions of the first tokens of '### Step x:'s and the last token position of the whole response).
        ref_resp_len: `List[float]`
            the reference response length for each response
    Returns:
        advantages: `(torch.Tensor)`
            shape: (bs, response_length)
        returns: `(torch.Tensor)`
    ...
    """
    #breakpoint()
    scores = token_level_rewards.data # (bs, response_length)
    id2score = defaultdict(list)
    ...
    for idx in id2score:
        assert len(id2score[idx]) > 1, "GRPO needs rollout.n > 1."
        group_score = torch.cat(id2score[idx], dim=0) # (rollout.n, response_length)
        id2mean[idx] = torch.mean(group_score)
        id2std[idx] = torch.std(group_score)
        id2len[idx] = group_score.shape[0]
     
    for i in range(bsz):
        scores[i][step_end_poss[i]] = (scores[i][step_end_poss[i]] - id2mean[index[i]]) / (id2std[index[i]] + eps)
        if normalize:
            scores[i][step_end_poss[i]] = scores[i][step_end_poss[i]] * 1.78 / (np.sqrt(id2len[index[i]]-1) + eps)
    ...
    # set the advantage of ### Step x: to 0
    max_len = returns.shape[1]
    for i in range(bsz):
        if len(delim_poss[i]) == 0:
            continue
        for j in range(5):
            poss = (torch.tensor(delim_poss[i])+j).clamp(max=max_len-1)
            returns[i][poss] = 0
    return returns, returns
...
def fill_left_nonzero(x: torch.Tensor) -> torch.Tensor:
    ...
    x_rev = torch.flip(x, dims=[1])                     # [R, C]
    C = x_rev.size(1)
    col_idx = torch.arange(C, device=x.device).expand_as(x_rev)  # [R, C]
    ...
    filled_rev = torch.where(last_nz_idx == -1,         
...
def compute_grpo_latent_advantage(
    ...
    """
    Compute advantage for VLPO, operating only on Outcome reward
    (with only one scalar reward for each response).
...
@torch.no_grad()
def compute_dapo_latent_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for DAPO with latent/hylar strategy, operating only on Outcome reward
    (with only one scalar reward for each response).
...
def compute_policy_loss(
    ...
    clipped_pg_loss_higher = torch.max(pg_loss, pg_loss2)  # clip if pg_loss < pg_loss2 (adv > 0)
...
def compute_kl(log_probs: torch.FloatTensor, ref_log_probs: torch.FloatTensor, kl_penalty: str) -> torch.Tensor:
    ...
    if kl_penalty == "low_var_kl":
        ...
        kl = (ref_log_probs - log_probs).clamp(-10.0, 10.0)
...
def compute_latent_vmf_kl(
    ...
    Compute the approximate KL divergence between two vMF distributions with the same κ:
        KL( vMF(μ_actor, κ) || vMF(μ_ref, κ) ) ≈ κ · (1 - μ_actor^T μ_ref)

    where μ_actor and μ_ref are L2-normalized unit vectors.
    KL=0 when directions are identical, KL=κ when orthogonal, KL=2κ when opposite.

    Args:
        mu_actor: (L, D) actor hidden states at latent positions
        mu_ref:   (L, D) ref model hidden states at latent positions
        kappa:    scalar, vMF concentration parameter

    Returns:
        kl: scalar tensor, mean vMF KL divergence per latent position
    """
    mu_a = mu_actor.float()
    mu_r = mu_ref.float()

    # Without L2 normalization, compute cosine similarity directly
    # cos_sim = (mu_a * mu_r).sum(dim=-1)       # (L,)
    # kl_per_pos = kappa * (1.0 - cos_sim)       # (L,)

    # After L2 normalization, cosine similarity is in [-1, 1], ensuring KL >= 0
    # Note: normalization is only for KL computation, does not affect original vectors in model forward
    # mu_a = F.normalize(mu_a, dim=-1)
    # mu_r = F.normalize(mu_r, dim=-1)
    cos_sim = (mu_a * mu_r).sum(dim=-1)       # (L,)
    kl_per_pos = kappa * (1.0 - cos_sim)       # (L,)
