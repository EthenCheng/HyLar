...
# hylar
from hylar_models.vllm.latent_recorder import LatentRecorder
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        if self.config.sampling_strategy in ["hylar"]:
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            if self.sampling_params.n > 1:
                ...
                # Also expand global_index if it was popped earlier
                if self.config.sampling_strategy in ["hylar"]:
                    # Repeat each global_index n times to match the expanded batch size
                    batch_sample_idx = [idx for idx in batch_sample_idx for _ in range(self.sampling_params.n)]

        # Put global_index back into non_tensor_batch if it was used
        if self.config.sampling_strategy in ["hylar"]:
            non_tensor_batch["global_index"] = np.array(batch_sample_idx)
    ...
    @torch.no_grad()
    def generate_sequences_hylar(self, prompts: DataProto) -> DataProto:
        # left-padded attention_mask
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            if self.sampling_params.n > 1:
                ...
                # Also expand global_index to match the expanded batch size
                batch_sample_idx = [idx for idx in batch_sample_idx for _ in range(self.sampling_params.n)]

        # Put global_index back into non_tensor_batch
        non_tensor_batch["global_index"] = np.array(batch_sample_idx)
