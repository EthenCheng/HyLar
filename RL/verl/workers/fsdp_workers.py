...
"""
import hylar_rl_patch  # renamed: HyLar RL patch
...
from examples.reward_function.hylar_reward_function import extract_and_check as easyr1_hylar_extract_and_check
from examples.reward_function.hylar_reward_function import extract_and_check_api as easyr1_hylar_extract_and_check_api
from examples.reward_function.hylar_reward_function import rule_then_api_batch_judge
...
def replace_sketch_token_content(s: str) -> str:
    """Replace latent token content with <canvas> placeholder to avoid decoding artifacts."""
...
class FSDPWorker(Worker):
    def __init__(
        ...
        if self._is_rollout and "api" in self.config.rule_based_judge.judge_function_name:
            ...
            elif self.config.rule_based_judge.api_name == 'gpt-4o-mini':
                self.api_client = None  # Uses OpenAI-compatible HTTP API, no separate client needed
            elif self.config.rule_based_judge.api_name == 'gpt-5':
                self.api_client = None  # Uses OpenAI-compatible HTTP API, no separate client needed
            elif self.config.rule_based_judge.api_name == 'Qwen3_32B_local':
                self.api_client = None  # Uses locally deployed vLLM OpenAI-compatible API
            else:
    ...
    def _build_rollout(self) -> None:
        ...
        if self.config.rollout.sampling_strategy in ["hylar"]:
            self.hash_server = ray.get_actor(self.config.rollout.hylar.hash_server_name)
        else:
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            if mode == "test":
                ...
                if self.config.rollout.sampling_strategy in ["hylar"]:
                    output = self.rollout.generate_sequences_hylar(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
            elif mode == "train_rl_gen":
                if self.config.rollout.sampling_strategy == "greedy":
                    output = self.rollout.generate_sequences(prompts=prompts)
                elif self.config.rollout.sampling_strategy in ["hylar"]:
                    output = self.rollout.generate_sequences_hylar(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
            else:
    ...
    def compute_rule_based_judge(self, data: DataProto):
        ...
        if self.config.rule_based_judge.judge_function_name=="rule_then_api_batch_judge":
            for i in range(len(data)):
                ...
                # Decode and replace garbled latent region content with <canvas> placeholder
                response_str = replace_sketch_token_content(
            ...
            correctness = rule_then_api_batch_judge(
                questions=data.non_tensor_batch["problem"].tolist() if hasattr(data.non_tensor_batch["problem"], 'tolist') else list(data.non_tensor_batch["problem"]),
                preds=response_strs,
                gts=data.non_tensor_batch["ground_truth"].tolist() if hasattr(data.non_tensor_batch["ground_truth"], 'tolist') else list(data.non_tensor_batch["ground_truth"]),
                ...
                api_url=getattr(self.config.rule_based_judge, 'api_url', None),
                api_key=getattr(self.config.rule_based_judge, 'api_key', None),
            )
        else:
            for i in tqdm(range(len(data)), desc="Rule-based judge", position=position, disable=self.rank != 0):
                ...
                if self.config.rule_based_judge.judge_function == "./examples/reward_function/hylar_reward_function.py:extract_and_check":
                    correctness.append(easyr1_hylar_extract_and_check(response_str, ground_truth))
                elif self.config.rule_based_judge.judge_function == "./examples/reward_function/hylar_reward_function.py:extract_and_check_api":
                    correctness.append(easyr1_hylar_extract_and_check_api(question, response_str, ground_truth, self.api_client))
                else:
