...
class BatchFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        ...
        for i in range(len(data)):
            valid_response_ids = response_ids[i][: response_length[i]]
            # Decode all tokens (including those in latent regions), then replace garbled content
            if "hylar" in self.config.reward_function:
                response_str_=replace_sketch_token_content(self.tokenizer.decode(valid_response_ids, skip_special_tokens=False)).replace("<|endoftext|>", "").replace("<|im_end|>", "")
            else:
        ...
        try:
            ...
            # Add questions parameter if the reward function supports it
            if "questions" in sig.parameters and "problem" in data.non_tensor_batch:
                extra_kwargs["questions"] = data.non_tensor_batch["problem"]
            # Add ground_truths parameter if the reward function supports it
            if "ground_truths" in sig.parameters:
                extra_kwargs["ground_truths"] = ground_truth
        except Exception:
            pass
...
class SingleFunctionRuleBasedJudgeManager(FunctionRuleBasedJudgeManager):
    ...
    def compute_rule_based_judge(self, data: DataProtoItem) -> bool:
        ...
        # Decode all tokens without filtering latent regions
        response_str = self.tokenizer.decode(
            valid_response_ids, skip_special_tokens=self.config.skip_special_tokens
        )
