...
def union_tensor_dict(tensor_dict1: TensorDict, tensor_dict2: TensorDict) -> TensorDict:
    ...
    # Unlock tensor_dict1 if it is locked
...
def union_numpy_dict(tensor_dict1: Dict[str, NDArray], tensor_dict2: Dict[str, NDArray]) -> Dict[str, NDArray]:
    for key in tensor_dict2.keys():
        if key in tensor_dict1 and key != "multi_modal_inputs": # by qixun: we skip the check for equality for multi_modal_inputs, 
            # and overide the tensor_dict1 with tensor_dict2.
            # Since even for the same sample, 
            # the multi_modal_inputs from tensor_dict2 (from MCTS) may not be the same as they are tokenized in different text context
            # (for tensor_dict2, the image features are tokenized along with appended reasoning texts, while for tensor_dict1,
            # the image features are tokenized along with the original question prompt).
...
class DataProto:
    ...
    def __getitem__(self, item: Union[int, slice, list, np.ndarray]) -> Union["DataProto", "DataProtoItem"]:
        tensor_data = self.batch[item]
        non_tensor_data = {key: value[item] for key, value in self.non_tensor_batch.items()}
        is_batch = (
            isinstance(item, slice) or
            (isinstance(item, (list, np.ndarray)) and not np.isscalar(item)) or
            (hasattr(item, 'dtype') and hasattr(item, '__len__'))  # torch.Tensor, etc.
        )
        return_type = DataProto if is_batch else DataProtoItem
    ...
    def concat(data: List["DataProto"]) -> "DataProto":
        ...
        for batch in data:
            if batch.batch is None:
                break
            '''if 'prompts' in batch.batch and 'responses' in batch.batch and 'response_mask' in batch.batch:
                if batch.batch['prompts'].shape[1]>2048 or batch.batch['responses'].shape[1]>2048 or batch.batch['response_mask'].shape[1]>2048:
                    breakpoint()'''
