...
def dataset_name_from_path(data_path: str) -> str:
    if "math3k" in data_path or "geometry3k" in data_path:
        ...
        return "Thyme-val"
    elif "codedance_RL" in data_path and 'val' not in data_path:
        return "CodeDance-train"
    elif "codedance_RL" in data_path and 'val' in data_path:
        return "CodeDance-val"
    else:
        # Generic handling: extract dataset name from path
        path_parts = data_path.rstrip('/').split('/')
        if 'train' in data_path and 'val' not in data_path:
            return f"{path_parts[-1]}-train"
        elif 'val' in data_path:
            return f"{path_parts[-1]}-val"
        else:
            return path_parts[-1]

def b64_to_pil(s: Union[str, bytes]) -> Image.Image:
    ...
    Args:
        s: Base64 encoded image data (str or bytes), or raw image binary data (bytes)
    """
    # If it's bytes, check if it's raw image data or base64
    if isinstance(s, bytes):
        # Check if it's raw image data by looking for common image file signatures
        # JPEG: starts with 0xFF 0xD8
        # PNG: starts with 0x89 0x50 0x4E 0x47
        # GIF: starts with 0x47 0x49 0x46
        if len(s) > 2 and (
            s[:2] == b'\xff\xd8' or  # JPEG
            s[:4] == b'\x89PNG' or    # PNG
            s[:3] == b'GIF'           # GIF
        ):
            # It's raw image binary data, directly open it
            return Image.open(io.BytesIO(s))
        
        # Otherwise, try to decode as base64 bytes
        try:
            s = s.decode('utf-8')
        except UnicodeDecodeError:
            # If decode fails, try to treat it as base64 bytes directly
            try:
...
class RLHFDataset(Dataset, ImageProcessMixin):
    ...
    def _build_messages(self, example: Dict[str, Any]) -> List[Dict[str, Any]]:
        # Get raw prompt and apply format_prompt if available
        prompt_str: str = example[self.prompt_key]
        
        # Apply format_prompt template if provided
        if self.format_prompt:
            prompt_str = Template(self.format_prompt).render(content=prompt_str)
        
        # System prompt for rollout and validation with structured reasoning format
        system_prompt = """
...
"""
    ...
    def _filter_overlong_and_invalid_prompts(self, example: Dict[str, Any]) -> bool:
        if 'geometry3k' not in self.data_path:
            ...
            elif 'codedance_RL' in self.data_path:
                # CodeDance dataset uses the same format as Thyme
                example = self._build_example(example, dataset_name="Thyme")
            else:
                # Try to build example with default Thyme format for unknown datasets
                try:
                    example = self._build_example(example, dataset_name="Thyme")
                except Exception as e:
                    print(f"Warning: Failed to build example for {self.data_path}: {e}")
                    return False
            if not example:
                return False
        ...
        if 'geometry3k' not in self.data_path:
            if 'Thyme-RL' in self.data_path:
                ...
                try:
                    ...
                    raise NotImplementedError(f"Dataset {self.data_path} not supported yet. Error: {e}")
